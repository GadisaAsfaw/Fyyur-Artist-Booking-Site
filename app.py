...
def venues():
  ...
  real_data = Area.query.all()
  for area in real_data:
    if len(area.venues)==0:
      real_data.remove(area)
  #print(real_data )
  #print(real_data [0].venues)
  #print(real_data [0].venues[0].genres)
  filtered_data = [
      {
        "city":area.city,
        "state":area.state,
        "venues":[{"id":venue.id,"name":venue.name} for venue in area.venues],
      }   for area in real_data]
...
def create_venue_submission():
  ...
  #genres = [Genre(name=name) for name in form.genres.data]
  error = False
  try:
      genres = []
